...
'Lead time value
LT = Cells(i - 2, 6).Value

inelastic_SS = Application.Sum(Range(Cells(i - 2, 9), Cells(i - 2, LT + 8)))
...
Skip_Col:
    ...
    'Need to repeat operations for this part?
    If SS_add > inelastic_SS Then
        k = 0
        Cells(i - 2, 4).Value = Cells(i - 2, 4).Value + 1
        GoTo RepeatLoop
    End If
